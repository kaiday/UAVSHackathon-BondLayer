...
const emptyDraft: Draft = { business: "Harbor Tech", registration: "", contact: "", verification: "", data: "", catalogue: "", policies: {}, membershipOffer: "", accepted: false, submitted: false };
...
export function OnboardingWizard() {
  ...
  if (draft.submitted) return <section className={styles.complete}><span><Check size={27} strokeWidth={2.5} /></span><p>Onboarding submitted</p><h1>Your setup is ready for review.</h1><div>We will let you know when your merchant workspace is ready to publish.</div><Link href="/">Return to dashboard</Link></section>;
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.layout}>
      ...
      <section className={styles.card}>
        ...
        {step === "readiness" && <div className={styles.stage}><p>Step 5 of 8</p><h2>Review catalogue readiness</h2><div>We will prepare your catalogue for agent discovery. Review the items that need your attention before publishing.</div>{draft.catalogue ? <><section className={styles.readinessSummary}><article><strong>48</strong><small>Products detected</small></article><article><strong>42</strong><small>Ready to publish</small></article><article className={styles.attention}><strong>6</strong><small>Need attention</small></article></section><section className={styles.readinessIssues}><h3>What needs your review</h3><button onClick={() => setStep("catalogue")}><span><CircleAlert size={14} strokeWidth={2.4} /></span><p><b>4 products are missing a category</b><small>Add a category so agents can compare them accurately.</small></p><i><ChevronRight size={18} /></i></button><button onClick={() => setStep("catalogue")}><span><CircleAlert size={14} strokeWidth={2.4} /></span><p><b>2 products are missing availability</b><small>Confirm stock status before these products can be published.</small></p><i><ChevronRight size={18} /></i></button><aside><b>What BondLayer can prepare</b><small>We can map recognised fields and flag incomplete information. Nothing is published until you approve it.</small></aside></section></> : <section className={styles.emptyReview}><span><Upload size={18} /></span><h3>Upload a catalogue to see readiness results.</h3><p>Once your import is processed, this step will show recognised products, mapped fields and anything that needs your review.</p><button onClick={() => setStep("catalogue")}>Go to catalogue upload</button></section>}<Footer back={back} next={next} /></div>}        {step === "policies" && <div className={styles.stage}><p>Step 6 of 8</p><h2>Publish your merchant policies</h2><div>Clear policies make buying more reliable for customers and agents.</div><div className={styles.policyList}>{["Shipping policy", "Returns policy", "Privacy policy", "Terms of sale"].map((policy, i) => <FilePicker key={policy} title={policy} hint={i < 3 ? "PDF or policy text" : "Optional - PDF or policy text"} required={i < 3} file={draft.policies[policy] ?? ""} onPick={(file) => patch({ policies: { ...draft.policies, [policy]: file } })} />)}</div><Footer back={back} next={next} /></div>}
